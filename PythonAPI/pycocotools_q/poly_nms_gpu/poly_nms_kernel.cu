...
// solve for the intersection point of 
__device__ inline int lineCross(float2 a,float2 b,float2 c,float2 d,float2&p){
    float s1,s2;
    s1=cross(a,b,c);
    s2=cross(a,b,d);
    if(sig(s1)==0&&sig(s2)==0) return 2;        // both equal to 0, meaning abcd is on the same line
    if(sig(s2-s1)==0) return 0;                 // meaning c & d is the same point
    p.x=(c.x*s2-d.x*s1)/(s2-s1);                // solving the linear equations Ax=b
...
__device__ inline void polygon_cut(float2*p,int&n,float2 a,float2 b, float2* pp){
    ...
    // n = 3
    for(int i=0;i<n;i++){
        // if vector ap is on the left-side of ab, add point p to intersection pp.
        if(sig(cross(a,b,p[i]))>0) pp[m++]=p[i];
        // if vector ab is not parallel to pp', solve for intersection between ab and pp' and save to pp
...
//返回三角形oab和三角形ocd的有向交面积, o是原点, 求3阶行列式//
...
__global__ void poly_nms_kernel(const int n_polys, const float nms_overlap_thresh,
    ...
    int dev_box_idx = threadsPerBlock * col_start + threadIdx.x;
    if (threadIdx.x < col_size) {
        block_polys[threadIdx.x * 9 + 0] = dev_polys[dev_box_idx * 9 + 0];
        block_polys[threadIdx.x * 9 + 1] = dev_polys[dev_box_idx * 9 + 1];
        block_polys[threadIdx.x * 9 + 2] = dev_polys[dev_box_idx * 9 + 2];
        block_polys[threadIdx.x * 9 + 3] = dev_polys[dev_box_idx * 9 + 3];
        block_polys[threadIdx.x * 9 + 4] = dev_polys[dev_box_idx * 9 + 4];
        block_polys[threadIdx.x * 9 + 5] = dev_polys[dev_box_idx * 9 + 5];
        block_polys[threadIdx.x * 9 + 6] = dev_polys[dev_box_idx * 9 + 6];
        block_polys[threadIdx.x * 9 + 7] = dev_polys[dev_box_idx * 9 + 7];
        block_polys[threadIdx.x * 9 + 8] = dev_polys[dev_box_idx * 9 + 8];
    }
    __syncthreads();
...
void _poly_nms(int* keep_out, int* num_out, const float* polys_host, int polys_num,
    ...
    CUDA_CHECK(cudaMalloc((void**)&polys_dev, polys_num * polys_dim * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(polys_dev,
                          polys_host,
                          polys_num * polys_dim * sizeof(float),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc((void**)&mask_dev, polys_num * col_blocks * sizeof(unsigned long long)));

    dim3 blocks(DIVUP(polys_num, threadsPerBlock), DIVUP(polys_num, threadsPerBlock));
    dim3 threads(threadsPerBlock);
    poly_nms_kernel<<<blocks, threads>>>(polys_num, nms_overlap_thresh, polys_dev, mask_dev);
